...
{
	 /* Setting GPIO functionality for selected 3 GPIOs for connecting leds */
...
void vPinmuxSel(LPC_GPIO_TypeDef* uiGPIOx, uint8_t ucPinnum, uint8_t ucfunctionality)
{
  ...
  */
  ...
  /* Calculating the pinsel number using the port number.
   * Pinsel number will be always 2 times port number,
   * but if the pin number is greater than 15 we have add one to it.
   */
  pinselnum = portnum * 2;
   if(ucPinnum>15)
	   pinselnum++;
